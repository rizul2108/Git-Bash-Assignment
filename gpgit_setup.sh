...
count=$(gpg --list-keys --with-colons | grep -c "^pub")

if [ "$count" -gt 1 ]; then
  gpg --list-keys --with-colons | awk -F: '/^pub:/ { print $5 }' | awk 'BEGIN{FS="\n"; RS=""} {print NR".", $0}'
  echo "Enter the number of the key you want to use for Git signing:"
  read -r key_number
  GPG_KEY_ID=$(gpg --list-keys --with-colons | awk -F: '/^pub:/ { print $5 }' | awk 'BEGIN{FS="\n"; RS=""} NR=='"$key_number"' {print $0}')
